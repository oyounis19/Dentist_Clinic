...
class Vite implements Htmlable
{
    ...
    public function __invoke($entrypoints, $buildDirectory = null)
    {
        ...
        if ($this->isRunningHot()) {
            return new HtmlString(
                $entrypoints
                    ->prepend('@vite/client')
                    ->map(fn ($entrypoint) => $this->makeTagForChunk($entrypoint, $this->hoturl($entrypoint), null, null))
                    ->join('')
            );
        }
     ...
     * @return string
     */
    protected function hoturl($asset)
    {
     ...
     * @return string
     */
    public function url($asset, $buildDirectory = null)
    {
        $buildDirectory ??= $this->buildDirectory;

        if ($this->isRunningHot()) {
            return $this->hoturl($asset);
        }
